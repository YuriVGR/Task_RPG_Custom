...
interface TaskCardProps {
    ...
    editTask: (id: string, title: string, description: string, status: number) => void;
}
...
const TaskCard: React.FC<TaskCardProps> = ({ title, description, status, id, removeTask, editTask }) => {
    ...
    return (
        <View className={`${statusColor} ${borderColor} m-4 rounded-lg p-4 border-2`}>
            ...
            <View className="flex-row p-2">
                ...
                <TouchableOpacity className="px-2 ml-auto" onPress={openDeleteModal}>
            ...
            <TaskDeleteModal isVisible={deleteModalVisible} onClose={closeDeleteModal} onDelete={() => removeTask(id)} />
            <TaskEditModal
